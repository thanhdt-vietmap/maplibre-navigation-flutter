...
import 'package:maplibre_navigation_flutter/extension.dart';
...
import 'package:maplibre_navigation_flutter/models/method_channel_event.dart';
...
class MapNavigationViewController {
  ...
  Future<bool> buildRoute({
    ...
    if (Platform.isIOS && waypoints.length > 3 && options?.mode != null) {
      assert(
        options!.mode != MapNavigationMode.drivingWithTraffic,
  ...
  Future<bool?> startNavigation({MapOptions? options}) async {
    ...
    final result = await _methodChannel.invokeMethod(
      MethodChannelEvent.startNavigation,
      args,
    );
  ...
  Future<bool?> removeMarkers(List<int> markerIds) async {
    return _methodChannel.invokeMethod(MethodChannelEvent.removeMarkers, {
      'markerIds': markerIds,
    });
  }
  ...
  Future<bool?> mute(bool isMute) async {
    return _methodChannel.invokeMethod(MethodChannelEvent.mute, {
      'isMute': isMute,
    });
  }
  ...
  Future<bool?> finishNavigation() async {
    var success = await _methodChannel.invokeMethod(
      MethodChannelEvent.finishNavigation,
      null,
    );
    return success;
  }
  ...
  Stream<RouteEvent>? get _streamRouteEvent {
    _onRouteEvent ??= _eventChannel.receiveBroadcastStream().map(
      (dynamic event) => _parseRouteEvent(event),
    );
