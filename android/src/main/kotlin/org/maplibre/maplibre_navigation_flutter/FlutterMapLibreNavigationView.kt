...
import org.maplibre.models.CurrentCenterPoint
...
class FlutterMapLibreNavigationView :
    ...
    constructor(
        ...
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.R) {
            this.activity = MaplibreNavigationFlutterPlugin.currentActivity!!
        } else {
            if (activity != null) {
        ...
        options =
            MapLibreMapOptions.createFromAttributes(context).compassEnabled(false).logoEnabled(true)
    ...
    companion object {
        ...
        var navigationVoiceUnits = IMPERIAL
        var zoom = 20.0
        var bearing = 0.0
        var tilt = 0.0
...
//                }
            ...
            "animateCamera" -> {
                ...
                animateCamera(
                    location,
                    methodCall.argument("bearing") as? Float?,
                    methodCall.argument("duration") ?: 1000,
                    methodCall.argument("zoom") as? Double?,
                    methodCall.argument("tilt") as? Double?
                )
            }
...
//                    navigationMode = DirectionsCriteria.PROFILE_DRIVING
        ...
        if (units != null) {
            if (units == "imperial") navigationVoiceUnits = IMPERIAL
            else if (units == "metric") navigationVoiceUnits = METRIC
        }
...
//                    routeStyleRes,
    ...
    private fun moveCameraWithoutAnimation(
        location: LatLng,
        bearing: Float?,
        zoom: Double?,
        tilt: Double?
...
//                speechPlayer?.onDestroy()
    ...
    override fun onListen(arguments: Any?, events: EventChannel.EventSink?) {
        MaplibreNavigationFlutterPlugin.eventSink = events
    }

    override fun onCancel(arguments: Any?) {
        MaplibreNavigationFlutterPlugin.eventSink = null
    }
    ...
    private fun addMarker(call: MethodCall, result: MethodChannel.Result) {
        ...
        try {
            ...
            data.entries.forEach() {
                ...
                val icon =
                    iconBitmap?.let { it1 -> IconFactory.getInstance(context).fromBitmap(it1) }
    ...
    override fun onResponseReceived(response: DirectionsResponse?, routeProgress: RouteProgress?) {
        if (response != null) {
            if (response.routes().isNotEmpty()) {
                ...
                PluginUtilities.sendEvent(
                    VietMapEvents.ROUTE_BUILT, "${currentRoute?.toJson()}"
                )
            }
        }
    }
